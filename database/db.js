...
const connectionString = process.env.DATABASE_URL || 'postgresql://postgres:10011@localhost:5432/skatepark'
...
const getUsersDB = async () => {
    ...
    try{
        const respuesta = await client.query("SELECT id,nombre,email,anios_experiencia,especialidad,foto,estado FROM skaters;")
        return {
            ok:true,
            msg: respuesta.rows,
        }
...
const createUserDB = async (nombre,hashPassword,email,experiencia,especialidad,pathFoto) => {
    const client = await pool.connect()
    const query = {
        text: "INSERT INTO skaters (nombre,email,password,anios_experiencia,especialidad,foto) VALUES($1,$2,$3,$4,$5,$6) RETURNING*",
